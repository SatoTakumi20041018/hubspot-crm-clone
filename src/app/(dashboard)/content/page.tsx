        ...
        <CardHeader>
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-2">
              ...
              <CardTitle>Content Remix - AIコンテンツリパーパス</CardTitle>
            </div>
          ...
          <CardDescription>
            既存コンテンツをAIが自動的に複数のフォーマットに変換します
          </CardDescription>
        </CardHeader>
        <CardContent>
          <div className="space-y-4">
            {contentRemixItems.map((item) => (
              <div key={item.id} className="rounded-lg border border-gray-200 p-4">
                <div className="flex items-start justify-between">
                  <div>
                    <p className="font-medium text-gray-900">{item.original}</p>
                    <Badge variant="info" className="mt-1">{item.type}</Badge>
                  </div>
                  <Button variant="outline" size="sm">リミックス実行</Button>
                </div>
                <div className="mt-3 flex flex-wrap gap-2">
                  {item.remixes.map((remix) => (
                    <span key={remix} className="rounded-full bg-gray-100 px-3 py-1 text-xs text-gray-600">
                      {remix}
                    </span>
                  ))}
                </div>
      ...
      <div className="grid grid-cols-1 gap-6 lg:grid-cols-2">
        ...
        <Card>
          <CardHeader>
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-2">
                <PenTool className="h-5 w-5 text-[#ff4800]" />
        ...
        <Card>
          <CardHeader>
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-2">
                <Globe className="h-5 w-5 text-[#ff4800]" />
                <CardTitle>ウェブサイトページ</CardTitle>
              </div>
              ...
              {websitePages.map((page, i) => (
                <div key={i} className="flex items-center justify-between rounded-lg border border-gray-100 p-3">
                  <div>
                    ...
                    <p className="text-xs text-gray-500">{page.url} - 最終更新: {page.lastUpdated}</p>
                  </div>
                  <div className="text-right">
        ...
        <Card>
          <CardHeader>
            <div className="flex items-center gap-2">
              <Headphones className="h-5 w-5 text-[#ff4800]" />
              ...
              {podcasts.map((podcast, i) => (
                <div key={i} className="flex items-center justify-between rounded-lg border border-gray-100 p-3">
                  <div>
                    ...
                    <p className="text-xs text-gray-500">{podcast.date} - {podcast.duration}</p>
                  </div>
                  <div className="text-right">
                    <p className="text-sm font-semibold text-gray-900">{podcast.plays.toLocaleString()}</p>
                    <p className="text-xs text-gray-500">再生</p>
                  </div>
                </div>
              ))}
            </div>
        ...
        <Card>
          <CardHeader>
            <div className="flex items-center gap-2">
              <Palette className="h-5 w-5 text-[#ff4800]" />
              <CardTitle>ブランドボイス設定</CardTitle>
            </div>
          </CardHeader>
          <CardContent className="space-y-4">
            <div className="rounded-lg bg-gray-50 p-4">
              <p className="text-sm font-medium text-gray-700">トーン</p>
              <p className="mt-1 text-sm text-gray-600">プロフェッショナルでありながら親しみやすい</p>
            </div>
            <div className="rounded-lg bg-gray-50 p-4">
              <p className="text-sm font-medium text-gray-700">ペルソナ</p>
              <p className="mt-1 text-sm text-gray-600">BtoB SaaS企業のマーケティング・営業担当者</p>
            </div>
            <div className="rounded-lg bg-gray-50 p-4">
              <p className="text-sm font-medium text-gray-700">キーワード</p>
              <div className="mt-1 flex flex-wrap gap-1.5">
                {["DX推進", "業務効率化", "データドリブン", "ROI最大化", "カスタマーサクセス"].map((kw) => (
