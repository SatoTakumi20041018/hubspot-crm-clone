...
const documents: Document[] = [
  { id: "doc1", name: "HubSpot CRM 製品概要資料", type: "プレゼン", views: 245, lastViewed: "2026-03-14", sharedWith: ["田中 太郎", "鈴木 花子", "山田 一郎"], size: "4.2 MB", createdAt: "2026-01-15", owner: "佐藤 匠" },
  { id: "doc2", name: "ECサイト構築 提案書", type: "提案書", views: 18, lastViewed: "2026-03-13", sharedWith: ["田中 太郎"], size: "2.8 MB", createdAt: "2026-02-20", owner: "佐藤 匠" },
  { id: "doc3", name: "料金プラン比較表", type: "PDF", views: 432, lastViewed: "2026-03-14", sharedWith: ["高橋 健一", "佐々木 美咲", "渡辺 大輔", "伊藤 さくら"], size: "1.1 MB", createdAt: "2026-01-08", owner: "田村 愛" },
  { id: "doc4", name: "クラウド移行サービス概要", type: "ホワイトペーパー", views: 89, lastViewed: "2026-03-12", sharedWith: ["鈴木 花子"], size: "3.5 MB", createdAt: "2026-02-01", owner: "佐藤 匠" },
  { id: "doc5", name: "太陽コーポレーション 導入事例", type: "ケーススタディ", views: 156, lastViewed: "2026-03-11", sharedWith: ["小林 誠", "渡辺 大輔"], size: "2.2 MB", createdAt: "2026-02-15", owner: "佐藤 匠" },
  { id: "doc6", name: "基幹システムリプレイス 契約書ドラフト", type: "契約書", views: 5, lastViewed: "2026-03-10", sharedWith: ["渡辺 大輔"], size: "890 KB", createdAt: "2026-03-05", owner: "田村 愛" },
  { id: "doc7", name: "データ分析基盤 技術仕様書", type: "PDF", views: 32, lastViewed: "2026-03-09", sharedWith: ["佐々木 美咲"], size: "5.1 MB", createdAt: "2026-03-01", owner: "佐藤 匠" },
];
...
export default function DocumentsPage() {
  ...
  const filtered = documents.filter((doc) => doc.name.toLowerCase().includes(search.toLowerCase()));
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="w-72"><Input variant="search" placeholder="ドキュメント名で検索..." value={search} onChange={(e) => { setSearch(e.target.value); setCurrentPage(1); }} /></div>

      {filtered.length === 0 && (
        <div className="flex flex-col items-center justify-center py-16 text-center">
          <div className="h-16 w-16 rounded-full bg-gray-100 flex items-center justify-center mb-4"><FileText className="h-8 w-8 text-gray-300" /></div>
          ...
          <Button size="sm" onClick={() => alert("作成は準備中です")}><Plus className="h-4 w-4 mr-1" /> ドキュメントを作成</Button>
        </div>
      )}
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-gray-200 bg-gray-50">
                ...
                <th className="px-4 py-3 text-left font-medium text-gray-500"><div className="flex items-center gap-1 cursor-pointer hover:text-gray-700" onClick={() => handleSort("name")}>ドキュメント名<ArrowUpDown className="h-3 w-3" /></div></th>
                <th className="px-4 py-3 text-left font-medium text-gray-500"><div className="flex items-center gap-1 cursor-pointer hover:text-gray-700" onClick={() => handleSort("type")}>タイプ<ArrowUpDown className="h-3 w-3" /></div></th>
                <th className="px-4 py-3 text-right font-medium text-gray-500"><div className="flex items-center justify-end gap-1 cursor-pointer hover:text-gray-700" onClick={() => handleSort("views")}>閲覧数<ArrowUpDown className="h-3 w-3" /></div></th>
            ...
            <tbody>
              {paginatedItems.map((doc) => (
                <tr key={doc.id} className={`border-b border-gray-100 hover:bg-gray-50 transition-colors ${selectedIds.has(doc.id) ? "bg-blue-50" : ""}`}>
                  ...
                  <td className="px-4 py-3 text-right"><div className="flex items-center justify-end gap-1 text-gray-600"><Eye className="h-3 w-3" />{doc.views}</div></td>
                  <td className="px-4 py-3 text-gray-600 text-xs"><div className="flex items-center gap-1"><Clock className="h-3 w-3" />{doc.lastViewed}</div></td>
      ...
      {selectedIds.size > 0 && (
        <div className="fixed bottom-6 left-1/2 -translate-x-1/2 z-50 flex items-center gap-3 rounded-lg border bg-white px-5 py-3 shadow-lg">
          ...
          <Button size="sm" variant="outline" onClick={() => alert("ダウンロードは準備中です")}><Download className="h-4 w-4 mr-1" />ダウンロード</Button>
