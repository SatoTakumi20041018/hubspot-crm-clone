...
import { ChevronRight, ArrowLeft, Eye, ThumbsUp, BookOpen } from "lucide-react";
...
const statusBadgeVariant = (status: string) => {
  switch (status) {
    ...
    case "REVIEW": case "レビュー中": return "warning" as const;
  ...
  useEffect(() => {
    fetch(`/api/knowledge-base`)
      ...
      .then((d) => {
        ...
        const found = Array.isArray(items) ? items.find((item: Article) => item.id === params.id) : null;
        setData(found || null);
        setLoading(false);
      })
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center gap-2 text-sm text-gray-500">
        <Link href="/knowledge-base" className="hover:text-gray-700">ナレッジベース</Link>
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                <p className="text-2xl font-bold text-gray-900">
                  {data.helpfulRate ? `${data.helpfulRate}%` : data.helpfulCount ? data.helpfulCount.toLocaleString() : "-"}
                </p>
                <p className="text-xs text-gray-500">役立ち度</p>
              </div>
            </div>
          </CardContent>
        </Card>
        ...
        <CardContent>
          <dl className="grid grid-cols-1 gap-4 sm:grid-cols-2">
            ...
            <div>
              ...
              <dd className="mt-1"><Badge variant="default">{data.category}</Badge></dd>
            </div>
            <div>
      ...
      {data.body && (
        <Card>
          <CardHeader><CardTitle>本文</CardTitle></CardHeader>
          <CardContent>
