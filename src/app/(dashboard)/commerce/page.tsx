    ...
    <div className="space-y-6">
      <PageHeader
        ...
        actions={
          <div className="flex gap-2">
            <Link href="/invoices"><Button variant="outline" size="sm"><FileText className="h-4 w-4 mr-1" />請求書一覧</Button></Link>
            <Link href="/payments"><Button size="sm"><CreditCard className="h-4 w-4 mr-1" />支払い一覧</Button></Link>
          </div>
        }
      />
      ...
      <div className="grid grid-cols-1 gap-6 lg:grid-cols-2">
        <Card>
          ...
          <CardContent><div className="space-y-3">{recentPayments.map((payment) => (<div key={payment.id} className="flex items-center justify-between rounded-lg border border-gray-100 p-3"><div><p className="text-sm font-medium text-gray-900">{payment.customer}</p><p className="text-xs text-gray-500">{payment.date} - {payment.method}</p></div><div className="text-right"><p className="text-sm font-semibold text-gray-900">¥{payment.amount.toLocaleString()}</p><Badge variant={payment.status === "completed" ? "success" : "warning"}>{payment.status === "completed" ? "完了" : "処理中"}</Badge></div></div>))}</div></CardContent>
        </Card>
        ...
        <Card>
          ...
          <CardContent><div className="space-y-3">{recentInvoices.map((invoice) => (<div key={invoice.id} className="flex items-center justify-between rounded-lg border border-gray-100 p-3"><div><p className="text-sm font-medium text-gray-900">{invoice.customer}</p><p className="text-xs text-gray-500">{invoice.id} - 期限: {invoice.dueDate}</p></div><div className="text-right"><p className="text-sm font-semibold text-gray-900">¥{invoice.amount.toLocaleString()}</p><Badge variant={invoiceStatusConfig[invoice.status].variant}>{invoiceStatusConfig[invoice.status].label}</Badge></div></div>))}</div></CardContent>
        </Card>
        ...
        <Card>
          <CardHeader><div className="flex items-center justify-between"><CardTitle>支払いリンク</CardTitle><Button variant="outline" size="sm"><LinkIcon className="h-4 w-4 mr-1" />リンク作成</Button></div></CardHeader>
